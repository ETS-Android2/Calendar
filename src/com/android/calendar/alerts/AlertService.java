...
/**
 * 이 service 는 달력일정 미리알림들을 띄우기 위해 리용된다.
 */
public class AlertService extends Service {

    public static final String ALERT_CHANNEL_ID = "my_alert_channel_01";// The id of the channel.
    ...
    static boolean updateAlertNotification(Context context) {
        ...
        NotificationMgr nm = createChannels(context);
    ...
    public static boolean generateAlerts(Context context, NotificationMgr nm,
        ...
        boolean alertEnabled = Utils.getAlertIsEnabled(context);
        if(!alertEnabled){
            return false;
        }
    ...
    private static void postNotification(NotificationInfo info, String summaryText,
        ...
        NotificationWrapper notification = AlertReceiver.makeExpandingNotification(context,
                ...
                info.endMillis, info.eventId, notificationId);

        String ringtone = NotificationPrefs.EMPTY_RINGTONE;

        //Play ringtone manually
        String path = Utils.getRingtonePreference(context);
        if(!path.isEmpty()) {
            Uri ringtoneURI = Uri.parse(path);
            Ringtone soundRingTone = RingtoneManager.getRingtone(context, ringtoneURI);
            soundRingTone.play();
        }

        addNotificationOptions(notification, true, tickerText,
     ...
     */
    private static void rescheduleMissedAlarms(ContentResolver cr, Context context,
    ...
    /**
     * Notification Channel을 창조한다.
     */
    public static NotificationMgr createChannels(Context context) {
    ...
    static class NotificationPrefs {
        ...
        private boolean getDefaultVibrate() {
            if (defaultVibrate < 0) {
                defaultVibrate = Utils.getDefaultVibrate(prefs) ? 1 : 0;
            }
