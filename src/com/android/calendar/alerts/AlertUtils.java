...
/**
 * 알림을 띄우는데 리용되는 정적성원들을 가지고 있는 Helper 클라스
 */
public class AlertUtils {
    ...
    //만료된 사건알림을 위해 하나의 Notification Id를 리용한다.
    public static final int EXPIRED_GROUP_NOTIFICATION_ID = 0;

    //알림 Service 를 기동시킬때 Extra 로 보내는 자료들의 Key상수값들
    ...
    static String formatTimeLocation(Context context, long startMillis, boolean allDay,
        ...
        time.setToNow();
        int today = Time.getJulianDay(time);
        time.set(startMillis);
        int eventDay = Time.getJulianDay(time);
    ...
    private static String getFiredAlertsKey(long eventId, long beginTime,
            long alarmTime) {
        return KEY_FIRED_ALERT_PREFIX + eventId +
                "_" +
                beginTime +
                "_" +
                alarmTime;
    ...
    /**
     * 두 날자사이의 날자수차이를 돌려준다.
     * @param startMillis 첫시간(미리초)
     * @param endMillis 둘째시간(미리초)
     * @param timeObj Time객체
     */
    private static int getIntervalInDays(long startMillis, long endMillis, Time timeObj) {
        timeObj.set(startMillis);
        int startDay = Time.getJulianDay(timeObj);
        timeObj.set(endMillis);
        return Time.getJulianDay(timeObj) - startDay;
    }
}
