...
import com.android.calendar.helper.AsyncQueryServiceHelper.OperationInfo;
...
public class AsyncQueryService extends Handler {
    ...
    private static final AtomicInteger mUniqueToken = new AtomicInteger(0);

    private final Context mContext;
    private Handler mHandler = this; // can be overridden for testing

    //Temporarily save start, end time and allDay flag when modifying recurrence events
    private long mStart = 0;
    private long mEnd = 0;
    private boolean mAllDay = false;
    private boolean mShowChangedTime = false;
    ...
    protected void onBatchComplete(int token, @Nullable Object cookie, ContentProviderResult[] results) {
        ...
        if(token == EditEventHelper.UPDATE_EVENT_TOKEN) {
            if(mShowChangedTime)
                CalendarController.SendEventUpdatedToEventInfoActivity(results, token, mAllDay, mStart, mEnd);
            else
                CalendarController.SendEventUpdatedToEventInfoActivity(results, token);
        }
    }

    public void setShowChangedTime(boolean allDay, long start, long end) {
        mShowChangedTime = true;
        mAllDay = allDay;
        mStart = start;
        mEnd = end;
    }

    public void unsetShowChangedTime() {
        mShowChangedTime = false;
    }
